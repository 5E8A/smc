...
- Root `package.json` (`@smc`, private) is a mix of workspace delegates (`dev`, `web:*`, `cms:*` wrappers) and direct `@scripts/` calls (`generate-lqip`, `check-content`, `sync-mods`, `sync-icons`); real per-workspace scripts live in each package. Workspace selectors use package names (`npm -w @smc/web …`).

## Commands

- `npm install` - install all workspaces into the single root `node_modules` (no per-package installs).
- `npm run dev` / `build` / `start` - site (delegate to `@smc/web`). `check-content` gate runs inside the build.
...
- **Base path defaults to `/smc`** but is configurable via `BASE_PATH` env var (`@shared/constants.ts`). Content stores paths base-agnostic (e.g. `/assets/content/...`); `assetPath()` and `ASSETS_BASE_PATH` resolve them at runtime. Typed links use `to="/post/$slug"` + `params`. `@web/dist/404.html` (spaFallback404 plugin in `@web/vite.config.ts`) makes GH Pages deep links work.
...
- **Markdown image titles**: `![alt](/path "title")` renders the title as a `<figcaption>` below the image via `ContentMarkdown`. Used for image captions in wiki content.
