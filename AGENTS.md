...
- `npm run generate-lqip` - rescans `@web/public/assets` and regenerates `@web/src/data/blurhash.json` (blurhash placeholders; transparent images skipped) plus `@web/src/data/media.json` (list of animated assets; warns about animated webp missing their `.static.webp` sibling). Runs automatically after CMS uploads.
